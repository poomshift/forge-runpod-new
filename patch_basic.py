...
def build_loaded(module, loader_name):
    ...
    def loader(*args, **kwargs):
        ...
        try:
            ...
            if "Can not safely load weights when explicit pickle_module is specified" in str(e):
                # Handle the specific pickle_module error
                if 'pickle_module' in kwargs:
                    del kwargs['pickle_module']
                kwargs['weights_only'] = True
                try:
                    result = original_loader(*args, **kwargs)
                except:
                    # If weights_only fails, try without it
